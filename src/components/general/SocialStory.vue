    ...
    <div
      ...
      <template v-if="!currentStepData.options">
        <div v-if="currentStepData.image"
             ref="image"
             class="relative lg:h-[400px] w-full overflow-hidden" aria-hidden="true">
        ...
        <p
            v-html="currentStepData.caption" class="h-[8rem] lg:h-[5rem] my-4 text-center step-description"
            aria-live="polite"
            aria-atomic="true"
        ...
        <div class="flex justify-around gap-4 w-full">
          ...
          >
            <div class="flex-[2] flex items-center justify-center  mb-2">
              <n-img :src="option.image" aria-hidden="true" class="w-full rounded-lg"/>
            </div>
            <div class="flex-1 flex flex-col justify-between">
              <button v-html="option.caption" class="option-button text-sm my-2 overflow-hidden line-clamp-3"></button>
              <span class="block mt-auto font-bold p-1 rounded transition-colors duration-300 hover:bg-mgrey-lighten-4"
...
const updateSRFocus = () => {
  nextTick(() => {
    const nextBtn = document.querySelector('.option-button')
    if (nextBtn) {
